(function ($) {
	...
	function init_island() {
		...
		$('.tip-island').hover(function () {
			$('#tiptip_content').text('台灣本島 與 台灣離島，需要分別建立自己的運送區域。台灣本島是一個運送區域，台灣離島是另一個運送區域，兩個區域可以有不同的運送方式&運費。若啟用此選項，即會將此運送區域改為 台灣離島的運送區域，並且在上一層運送區域的列表，請務必將離島區域，置放於本島區域的上面，才能正確啟用')
		})
	}
