...
namespace Woomp\GuzzleHttp;

use Woomp\GuzzleHttp\Cookie\CookieJar;
use Woomp\GuzzleHttp\Exception\GuzzleException;
use Woomp\GuzzleHttp\Promise;
use Woomp\GuzzleHttp\Psr7;
use Woomp\Psr\Http\Message\RequestInterface;
use Woomp\Psr\Http\Message\ResponseInterface;
use Woomp\Psr\Http\Message\UriInterface;
/**
...
class Client implements ClientInterface
{
     ...
     *
     * @see \Woomp\GuzzleHttp\RequestOptions for a list of available request options.
     */
    public function __construct(array $config = [])
    {
        ...
        if (isset($config['base_uri'])) {
            $config['base_uri'] = \Woomp\GuzzleHttp\Psr7\uri_for($config['base_uri']);
        }
    ...
    private function buildUri($uri, array $config)
    {
        ...
        $uri = \Woomp\GuzzleHttp\Psr7\uri_for($uri === null ? '' : $uri);
        if (isset($config['base_uri'])) {
            $uri = \Woomp\GuzzleHttp\Psr7\UriResolver::resolve(\Woomp\GuzzleHttp\Psr7\uri_for($config['base_uri']), $uri);
        }
    ...
    private function transfer(RequestInterface $request, array $options)
    {
        ...
        try {
            return \Woomp\GuzzleHttp\Promise\promise_for($handler($request, $options));
        } catch (\Exception $e) {
            return \Woomp\GuzzleHttp\Promise\rejection_for($e);
        }
    }
    /**
    ...
    private function applyOptions(RequestInterface $request, array &$options)
    {
        ...
        if (isset($options['form_params'])) {
            ...
            $options['_conditional'] = \Woomp\GuzzleHttp\Psr7\_caseless_remove(['Content-Type'], $options['_conditional']);
        ...
        if (isset($options['json'])) {
            $options['body'] = \Woomp\GuzzleHttp\json_encode($options['json']);
            unset($options['json']);
            // Ensure that we don't have the header in different case and set the new value.
            $options['_conditional'] = \Woomp\GuzzleHttp\Psr7\_caseless_remove(['Content-Type'], $options['_conditional']);
        ...
        if (!empty($options['decode_content']) && $options['decode_content'] !== true) {
            // Ensure that we don't have the header in different case and set the new value.
            $options['_conditional'] = \Woomp\GuzzleHttp\Psr7\_caseless_remove(['Accept-Encoding'], $options['_conditional']);
        ...
        if (isset($options['body'])) {
            ...
            $modify['body'] = \Woomp\GuzzleHttp\Psr7\stream_for($options['body']);
            unset($options['body']);
        }
        if (!empty($options['auth']) && is_array($options['auth'])) {
            ...
            switch ($type) {
                case 'basic':
                    // Ensure that we don't have the header in different case and set the new value.
                    $modify['set_headers'] = \Woomp\GuzzleHttp\Psr7\_caseless_remove(['Authorization'], $modify['set_headers']);
        ...
        $request = \Woomp\GuzzleHttp\Psr7\modify_request($request, $modify);
        if ($request->getBody() instanceof \Woomp\GuzzleHttp\Psr7\MultipartStream) {
