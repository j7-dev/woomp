...
class Logger {
	...
	public function __construct( string $sub_dir_name, string $log_file_name, array $opts = array() ) {
	...
	public function log( string $base_message, array $tags = array() ): void {
		$message = $this->transform_message( $base_message, $tags );
		file_put_contents( $this->log_file, $message, FILE_APPEND );
	}

	public function get_test_log( string $base_message = 'Test Message', array $tags = array() ): array {
		$info = array(
	...
	private function transform_message( string $original, $tags = array() ): string {
