...
class Field {
	...
	public function set_metabox() {
		...
		if ( ! $order_or_subscription->get_meta( '_ezpay_invoice_data' ) ) {
			$order_or_subscription->update_meta_data( '_ezpay_invoice_data', [] );
			$order_or_subscription->save();
		}

		$ezpay_invoice_data = (array) $order_or_subscription->get_meta( '_ezpay_invoice_data' );
