(function ($) {
	$( document ).ready(
		function ($) {
			...
			$(
				"#ezpay-donate-number,#ezpay-invoice-type,#ezpay-individual-invoice"
			).selectWoo();
			...
			$( "#ezpay-individual-invoice" ).change(
				function () {
					var individualOption = $(
						"#ezpay-individual-invoice option:selected"
					).text();
			 ...
			 * 檢查載具欄位要不要顯示
			 */
			function carrierNumberCheck() {
					const individualInvoiceValue = $( "#ezpay-individual-invoice" ).val();
					const SHOW_CONDITION         = ["自然人憑證", "手機條碼"];
				if (SHOW_CONDITION.includes( individualInvoiceValue )) {
					$( "#ezpay-carrier-number_field" ).show();
				} else {
					$( "#ezpay-carrier-number_field" ).hide();
				}
