...
final class RY_NewebPay_Shipping_admin {
	...
	public static function init() {
		...
		add_action( 'woocommerce_order_action_send_at_cvs_email', array( 'RY_NewebPay_Shipping', 'send_at_cvs_email' ) );

		add_action( 'add_meta_boxes', array( 'RY_NewebPay_Shipping_Meta_Box', 'add_meta_box' ), 40, 2 );
	}
	...
	public static function check_ship_destination() {
		global $wpdb;
		if ( 'billing_only' === get_option( 'woocommerce_ship_to_destination' ) ) {
			foreach ( array( 'ry_newebpay_shipping_cvs' ) as $method_id ) {
				$wpdb->update(
					$wpdb->prefix . 'woocommerce_shipping_zone_methods',
					array(
						'is_enabled' => 0,
